...
class HealthCheckServiceSpec extends AnyFlatSpec with Matchers {
  ...
  private val mockHealthCheckRepository = new HealthCheckRepository {
    override def getPostgresProcessId: IO[Option[String]] = IO.pure(Some("processId"))
  }

  "HealthCheckService" should "return HealthCheck with ok=true when Postgres process ID is available" in {
    val healthCheckService = new HealthCheckServiceImpl(mockHealthCheckRepository)

    val result = healthCheckService.status.unsafeRunSync()
    result shouldBe HealthCheck(PostgresStatus(ok = true))
  }
  ...
  it should "return HealthCheck with ok=false when Postgres process ID is not available" in {
    ...
    val healthCheckService = new HealthCheckServiceImpl(mockHealthCheckRepositoryWithoutProcessId)

    val result = healthCheckService.status.unsafeRunSync()

    result shouldBe HealthCheck(PostgresStatus(ok = false))
  }
  ...
  it should "return HealthCheck with ok=false and log the error when an exception occurs" in {
    val mockHealthCheckRepositoryWithError = new HealthCheckRepository {
      override def getPostgresProcessId: IO[Option[String]] = IO.raiseError(new RuntimeException("Some error"))
    }

    val healthCheckService = new HealthCheckServiceImpl(mockHealthCheckRepositoryWithError)
